...
def simulate_batch(settings):
    ...
    config["lepton injector"]["simulation"]["output name"] = "./output/custom_%d_output_LI.h5" % (
        id
    )
    ...
    config["detector"]["injection offset"] = [0.0, 0.0, 0]
...
def distribute_jobs(n_workers, n_events):
    ...
    for i in range(n_workers):
        ...
        process = Process(target=simulate_batch, args=(settings,))
