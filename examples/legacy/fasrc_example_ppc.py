...
def initialize_args():
    ...
    # Meta
    parser.add_argument("-n", dest="n", type=int, default=10, help="number of events to simulate.")
    parser.add_argument(
        "-s",
        "--seed",
        dest="seed",
    ...
    parser.add_argument(
        "--emin", dest="emin", type=float, default=1e3, help="Minimum energy for the simulation."
    )
    parser.add_argument(
...
def main(args):
    ...
    config["run"]["storage prefix"] = f"{args.output_prefix}/"
    config["run"]["outfile"] = f"{args.output_prefix}/{prefix}_photons.parquet"
    ...
    config["photon propagator"]["PPC_CUDA"]["paths"]["ppc_tmpdir"] = args.ppc_tmpfile.replace(
        ".ppc", f"{seed}.ppc"
    )
    config["photon propagator"]["PPC_CUDA"]["paths"]["ppc_tmpfile"] = args.ppc_tmpfile.replace(
        ".ppc", f"{seed}.ppc"
    )
    config["photon propagator"]["PPC_CUDA"]["paths"]["f2k_tmpfile"] = args.f2k_tmpfile.replace(
        ".f2k", f"{seed}.f2k"
    )
    config["photon propagator"]["PPC_CUDA"]["paths"]["location"] = (
