...
def prometheus_inj_to_li_injection_event(truth: ak.Record) -> LIInjectionEvent:
    ...
    initial_state = Particle(
        ...
        np.array([truth["initial_x"], truth["initial_y"], truth["initial_z"]]),
        np.array(
            [
    ...
    for idx, parent in enumerate(truth["parent"]):
        ...
        final_state = PropagatableParticle(
            ...
            np.array(
                [
                    np.cos(truth["final_state_azimuth", idx])
                    * np.sin(truth["final_state_zenith", idx]),
                    np.sin(truth["final_state_azimuth", idx])
                    * np.sin(truth["final_state_zenith", idx]),
