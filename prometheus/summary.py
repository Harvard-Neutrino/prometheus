...
def emit_run_summary(prom, outfile, end_out, size=None):
    ...
    try:
        ...
        inj_time = float(getattr(prom, "_end_inj", float("nan")) - getattr(prom, "_start_inj", float("nan")))
        prop_time = float(getattr(prom, "_end_prop", float("nan")) - getattr(prom, "_start_prop", float("nan")))
        write_time = float(getattr(prom, "_end_out", end_out) - getattr(prom, "_start_out", float("nan")))
        total_time = float(getattr(prom, "_end_out", end_out) - getattr(prom, "_run_start_time", float("nan")))
        ...
        if compact:
            ...
            header_lines.append(f"Propagator:    {getattr(config.photon_propagator, 'name', 'unknown')}")
            modules = getattr(prom.detector, 'n_modules', len(getattr(prom.detector, 'modules', [])))
            ...
            footer_lines.append("")
            footer_lines.append("✅ Simulation completed successfully" if success else "❌ Simulation failed")
            footer_lines.append("")
            ...
            init_noise = bool(getattr(prom, '_init_output', None) and getattr(prom, '_init_output', '').strip())
            inject_noise = bool(getattr(prom, '_inject_output', None) and getattr(prom, '_inject_output', '').strip())
            prop_noise = bool(getattr(prom, '_propagate_output', None) and getattr(prom, '_propagate_output', '').strip())
            ...
            if total_warnings + internal_noise_count > 0:
                if mode == 'user':
                    ...
                    print("  Hint: set config.run.summary_mode='debug' (and optionally config.run.verbosity='DEBUG') to inspect details")
                else:
                    logger.warning("⚠ Warnings detected (%d)", total_warnings + internal_noise_count)
        ...
        if summary_json and outfile:
            try:
                ...
                jdata = {
                    ...
                    'timings': {'inj': inj_time, 'prop': prop_time, 'write': write_time, 'total': total_time},
