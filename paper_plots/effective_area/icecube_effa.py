...
def initialize_args():
    ...
    # Meta
    parser.add_argument("-n", dest="n", type=int, default=100, help="number of events to simulate.")
    parser.add_argument(
        "-s",
        "--seed",
        dest="seed",
    ...
    parser.add_argument(
        "--emin", dest="emin", type=float, default=1e2, help="Minimum energy for the simulation."
    )
    parser.add_argument(
        "--emax", dest="emax", type=float, default=1e6, help="Maximum energy for the simulation."
    )
    parser.add_argument(
        "--gamma", dest="gamma", type=float, default=1.0, help="Spectral index for sampling"
    )
    parser.add_argument(
        "--force_volume",
        dest="force_volume",
...
def main(args):
    ...
    config["general"]["config location"] = (
        f"{args.output_prefix}/config_{args.final_1}_{args.final_2}_seed_{seed}.json"
    )
    ...
    config["lepton injector"]["simulation"]["output name"] = (
        f"{args.output_prefix}/config_{args.final_1}_{args.final_2}_seed_{seed}.h5"
    )
    ...
    config["lepton injector"]["location"] = (
        "/n/holylfs05/LABS/arguelles_delgado_lab/Lab/common_software/lib64/"
    )
    config["lepton injector"]["xsec location"] = (
        "/n/holylfs05/LABS/arguelles_delgado_lab/Lab/common_software/source/LeptonInjector/resources/"
    )
    config["lepton propagator"]["lepton"] = clepton
    config["photon propagator"]["storage location"] = (
        f"{args.output_prefix}/{args.final_1}_{args.final_2}_seed_{seed}_"
    )
    photo_prop = "PPC_CUDA"
    config["photon propagator"]["name"] = photo_prop
    config["photon propagator"][photo_prop]["ppc_tmpfile"] = args.ppc_tmpfile.replace(
        ".ppc", f"{seed}.ppc"
    )
    config["photon propagator"][photo_prop]["f2k_tmpfile"] = args.f2k_tmpfile.replace(
        ".f2k", f"{seed}.f2k"
    )
    config["photon propagator"][photo_prop]["location"] = (
        "/n/holylfs05/LABS/arguelles_delgado_lab/Lab/common_software/source/PPC_CUDA_new/"
    )
    config["photon propagator"][photo_prop]["ppctables"] = "/n/home12/jlazar/hebe/PPC_CUDA/"
    config["photon propagator"][photo_prop]["ppc_exe"] = (
        "/n/holylfs05/LABS/arguelles_delgado_lab/Lab/common_software/source/PPC_CUDA_new/ppc"
    )
